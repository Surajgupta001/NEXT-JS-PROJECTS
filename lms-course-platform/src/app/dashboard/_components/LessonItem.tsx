            ...
            <div className='flex items-center gap-2.5 w-full min-w-0'>
                <div className='shrink-0'>
                    {completed ? (
                        ...
                        <div className={cn('size-5 rounded-full border-2 bg-background flex justify-center items-center', isActive ? 'border-primary bg-primary/10 dark:bg-primary/20' : 'border-muted-foreground opacity-60')}>
                            <Play className={cn('size-2.5 fill-current', isActive ? 'text-primary' : 'text-muted-foreground')} />
                        </div>
                    )}
                </div>
                <div className='flex-1 text-left min-w-0'>
                    <p className={cn('text-xs font-medium truncate', completed ? 'text-green-800 dark:text-green-200' : isActive ? 'text-primary font-semibold' : 'text-foreground')}>
