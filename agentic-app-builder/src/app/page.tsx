...
import { BlueTitle, GrayTitle, SectionHeading, SectionLabel } from "@/components/Reuseable";
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] selection:bg-white/20">
      ...
      {/* HERO */}
      <section className="relative flex flex-col items-center px-4 pt-40 pb-24 overflow-hidden text-center">
        <HoleBackground
        ...
        <h1 className="z-10 max-w-3xl mx-auto font-serif text-5xl leading-tight tracking-tight text-balance sm:text-6xl lg:text-7xl">
          <GrayTitle>Forge your dream</GrayTitle>
          <br />
          <BlueTitle>from a single prompt.</BlueTitle>
        </h1>
        <p className="z-10 max-w-xl mx-auto mt-6 text-base leading-relaxed text-balance text-white/40">
        ...
        <div className="relative w-full max-w-2xl mx-auto mt-12">
          ...
          >
            <textarea
              ...
              rows={1}
              className="w-full px-5 pt-5 pb-4 text-sm bg-transparent resize-none placeholder:text-white/20 focus:outline-none sm:text-base"
            ...
            <div className="flex items-center justify-between border-t border-white/6 px-4 py-2.5">
              ...
              {isSignedIn ? (
                <Button
                  ...
                  className="h-8 px-5 font-semibold rounded-full"
                ...
                <SignInButton mode="modal">
                  <Button className="h-8 px-5 font-semibold bg-white rounded-full">
                    Generate
                    <ArrowRight className="h-3.5 w-3.5" />
                  </Button>
                </SignInButton>
      ...
      <section className="px-4 pb-32">
        <div className="mx-auto max-w-5xl overflow-hidden rounded-2xl border border-white/8 bg-[#0f0f0f] shadow-2xl shadow-black/60">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-white/6">
            ...
            <div className="flex items-center justify-center w-64 h-6 px-3 mx-auto rounded-md bg-white/5">
          ...
          <div className="flex h-105">
            ...
            <div className="flex flex-col flex-1">
              <div className="flex items-center gap-1 px-4 border-b border-white/6">
              ...
              <div className="flex flex-1 gap-3 overflow-hidden bg-[#141414] p-5">
                {["Todo", "In Progress", "Done"].map((col, ci) => (
                  <div key={col} className="flex flex-col w-1/3 gap-2">
      ...
      <section className="px-4 pb-32">
        ...
        <div className="grid max-w-5xl grid-cols-1 gap-px mx-auto overflow-hidden border rounded-2xl border-white/6 bg-white/6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(({ icon: Icon, label, desc }) => (
            <div
              key={label}
              className="group bg-[#0a0a0a] p-7 hover:bg-[#0f0f0f]"
            >
              <div className="flex items-center justify-center mb-4 border rounded-lg h-9 w-9 border-white/8 bg-white/4 group-hover:border-white/15 group-hover:bg-white/8">
                <Icon className="w-4 h-4 text-white/60 group-hover:text-blue-400/70" />
              </div>
          ...
          {STEPS.map((step, i) => (
            <div key={step.number} className="flex gap-6">
              <div className="flex flex-col items-center">
                <div className="flex items-center justify-center w-10 h-10 border rounded-full border-white/10 bg-white/4">
                ...
                {i < STEPS.length - 1 && (
                  <div className="w-px h-full mt-2 bg-white/6" />
                )}
              </div>
      ...
      <div className="max-w-5xl mx-auto text-center mb-14">
        ...
        <p className="max-w-sm mx-auto mt-4 text-sm text-white/35">
          No credit card required. Upgrade or downgrade anytime.
        </p>
      </div>

      <div className="grid max-w-5xl grid-cols-1 gap-4 mx-auto sm:grid-cols-3">
        {PRICING_PLANS.map((plan) => {
          ...
          return (
            ...
            >
              ...
              {/* Feature list */}
              <div className="pt-6 mb-8 space-y-3 border-t border-white/6">
                {plan.features.map((f) => (
              ...
              <div className="mt-auto">
                {isActive ? (
                  <Button
                    disabled
                    className="w-full text-sm font-semibold bg-transparent border rounded-full opacity-50 cursor-not-allowed border-white/10 text-white/60"
                    variant="ghost"
                  >
                    ✓ Current plan
                  </Button>
                  ...
                  isSignedIn ? (
                    ...
                    <SignInButton mode="modal">
                      <Button
                        className="w-full text-sm font-semibold bg-transparent border rounded-full border-white/10 text-white/60 hover:bg-white/6 hover:text-white/90"
                        variant="ghost"
                      >
                        Get started free
                        <ArrowRight className="h-3.5 w-3.5" />
      ...
      {/* CTA */ }
  <section className="relative max-w-5xl px-10 py-24 mx-auto mb-32 overflow-hidden text-center border rounded-2xl border-white/8">
    <HoleBackground
      strokeColor="rgba(255,255,255,0.05)" // blur
      numberOfLines={36}
      numberOfDiscs={36}
    ...
    <SignInButton mode="modal">
      <Button
        size="lg"
        className="relative px-8 bg-white rounded-full h-11"
      >
        Get started free
  ...
  {/* FOOTER */ }
  <footer className="relative z-10 flex flex-wrap items-center justify-center px-6 py-12 mx-auto border-t border-white/7 text-stone-400">
    Made with ❤️ by Forge. All rights reserved. &copy; {new Date().getFullYear()}
  </footer>
    </main >
  );
}
